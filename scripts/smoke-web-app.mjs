 ...
 *
 * The flow itself lives in `lib/mcp-app-flow.mjs`, shared with `pack:verify`
 * (#2003) — see that header for why it is shared rather than copied, and for
 * the `data-app-status="ready"` contract the assertion rests on.
 *
 ...
 * #2003, drives this same App flow against the **installed** bin. Keep both:
...
// The URL the test server announces on startup — authoritative over the
...
try {
  ...
  try {
    await Promise.race([server.whenChildExits(), driveAppFlow({ page, url })]);
  } catch (err) {
    const notes = [
