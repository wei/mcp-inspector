...
export function vitestSharedPaths(clientDir: string) {
  ...
  const nodeModulesAliases = [
    ...
    // above. Both are reached only through root-owned code that has no manifest
    ...
    // place a client's resolution chain is guaranteed to find them. Pointing
    // these at `<client>/node_modules` is what broke when the MCP packages moved
    // to the root (#1970): express was never declared by a client at all, it
    // arrived in `clients/cli` as a peer of `express-rate-limit` under
    // `@modelcontextprotocol/server-legacy`, so removing that manifest entry
    // took express with it and every cli test that spawns a test server failed
    // to resolve it.
    {
      find: /^express$/,
